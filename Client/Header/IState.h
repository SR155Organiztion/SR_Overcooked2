...
*			- IRawState : 재료가 생(날 것) 상태를 정의
*			- IChopState : 재료가 썰리는 중 상태를 정의
*			- ICookState : 재료가 가열 조리 중 상태를 정의
*			- IDoneState : 조리가 완료된 상태를 정의
*			- IBurtnState : 오버쿡되어 탄 상태를 정의
* 
...
class IBurntState : public IState {
	// IState을(를) 통해 상속됨
	void Enter_State(CIngredient* pIngredient) override;
	void Update_State(CIngredient* pIngredient, const _float& fTiemDelta) override;
	void Exit_State(CIngredient* pIngredient) override;
