...
, m_pSrcRect(0), m_eObjectType(END_OBJECT), m_dwStartTime(0), m_dwLimitTime(0), m_dwTime(0)
...
void CUi_Order::Get_Order(wstring _Name, float _time)
{
	...
	m_vPos = m_vStartPos;
	m_dwHideTime = GetTickCount64() + static_cast<DWORD>(_time * 100000); //타이머 시작
	m_vecType.push_back(_Name);
